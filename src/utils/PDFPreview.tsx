...
export const PDFPreview: React.FC<PDFPreviewProps> = ({ pdfDataUrl, className = "" }) => {
  ...
  const onDocumentLoadError = (error: Error) => {
    ...
    console.error('PDF data info:', typeof pdfData === 'string' ? `string length: ${pdfData.length}` : 'Uint8Array object');
    setIsLoading(false);
  };
        ...
        >
          {isLoading && (
            <div className="flex flex-col items-center justify-center h-full text-white bg-gray-800 rounded">
              ...
              <p className="text-sm text-gray-400">This may take a moment for larger files</p>
            </div>
          )}
          ...
          {!isLoading && !pdfData && (
            <div className="text-yellow-300 p-8 text-center bg-gray-800 rounded">
              ...
              <p className="text-sm text-gray-400">Converting PDF data for display</p>
              <p className="text-xs text-gray-500 mt-2">Original data length: {pdfDataUrl?.length || 'unknown'}</p>
            </div>
          )}
            ...
            <Document
              ...
              error={
                <div className="text-red-300 p-8 text-center bg-gray-800 rounded">
                  ...
                  <p className="text-sm text-gray-400 mb-4">This PDF format might not be supported by the browser viewer.</p>
                  <p className="text-xs text-gray-500">Try downloading the PDF directly using the button below.</p>
                </div>
              }
            >
              <Page
